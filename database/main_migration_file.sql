...
-- ============================================================
-- AGENT HARNESS: For n8n Agentic SQL Workflow
-- ============================================================
...
CREATE TABLE agent_sessions (
    ...
    available_datasets JSONB DEFAULT '[]'::jsonb,
    selected_datasets JSONB DEFAULT '[]'::jsonb,
    plan_name TEXT,
    plan_goal TEXT,
    plan_complexity TEXT CHECK (plan_complexity IN ('simple', 'moderate', 'complex')),
    plan_assumptions JSONB DEFAULT '[]'::jsonb,
    final_output_description TEXT,
    task_count INTEGER DEFAULT 0,
    completed_task_count INTEGER DEFAULT 0,
    failed_task_count INTEGER DEFAULT 0,
    status TEXT DEFAULT 'created' CHECK (status IN (
        'created', 'discovering', 'enhancing', 'planning', 'executing', 
...
'Agent analysis sessions for the agentic SQL workflow.';
...
CREATE TABLE agent_tasks (
    ...
    task_type TEXT NOT NULL CHECK (task_type IN (
        'analysis', 'sql_query', 'sql_aggregate', 'validation', 'transform'
    )),
    ...
    on_failure TEXT DEFAULT 'stop' CHECK (on_failure IN ('stop', 'retry', 'skip', 'human')),
    status TEXT DEFAULT 'pending' CHECK (status IN (
        'pending', 'waiting_dependency', 'running', 'completed', 'failed', 'skipped'
    )),
    result JSONB,
    result_summary TEXT,
    error_message TEXT,
...
CREATE INDEX idx_agent_tasks_session ON agent_tasks(session_id, task_order);
CREATE INDEX idx_agent_tasks_status ON agent_tasks(status);

COMMENT ON TABLE agent_tasks IS 
'Individual tasks within an agent session.';
